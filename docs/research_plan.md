...
- *The crossover does not replicate on another task.* Then the paper becomes "when it does and does not", still publishable if the OOD and dynamics claims hold; check after the second task's 300-question point (week 3). **Outcome (2026-09-13):** it did not replicate on Geometry3K; SFT wins at 3 of 4 budgets (one seed). The mechanism is diagnosable from the outputs: a third of OPD solutions never terminate within 1536 tokens, the teacher itself fails to terminate 22% of the time, OPD's rollout EOS rate never moves, while SFT's rejection-sampled data contains only terminating correct solutions. Conditional on producing an answer, OPD is at least as accurate. The paper's thesis therefore becomes: on-policy distillation transfers the teacher's behaviour including its failure modes; it wins when the teacher is reliable (ChartQA, teacher format 0.99) and loses to filtered SFT when the teacher is not (Geometry3K, teacher format 0.78). Controls run (2026-09-14, 4 seeds): seeds confirm the 900-problem gap (-7.9 [-10.0, -5.9]) and a tie at 300; SFT on *unfiltered* teacher samples falls to OPD's termination rate (0.62 vs 0.62 at 300) and loses its edge there, and explains half of the gap at 900 (+3.6 of 7.9). The residual is the rollout distribution itself: teacher samples at temperature 0.7 terminate 78% of the time, the student's temperature-1.0 rollouts 56%, and the KL objective never rewards stopping. Method B is therefore defined: `opd-term` (loss only on terminated rollouts, the on-policy analogue of rejection sampling) and `opd-t07` (rollouts at 0.7), both queued at 900 problems with 4 seeds. If either closes the gap, the paper has a positive method contribution that falls directly out of the diagnosis.
