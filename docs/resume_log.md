...
- Accomplished a seed-robust version of the low-data result, as measured by four seeds at 100 and three at 300 training questions (19 new training runs on L40S GPUs) giving OPD 0.811 (sd 0.009) versus SFT 0.793 (sd 0.022) at 100 questions and OPD 0.827 (sd 0.008) versus SFT 0.799 (sd 0.015) at 300, with paired bootstrap deltas of +1.8 [+0.4, +3.4] and +2.9 [+1.3, +4.6] points over the shared seeds, by running the sweep through the idempotent experiment runner and aggregating with `python -m vlm_opd.analysis.data_efficiency` (intervals pooled over questions across seeds, deltas paired by seed and question).
- Recorded the correction honestly: the original single-seed +5.8 at 300 questions was the most favourable of four SFT seeds (0.778 against 0.800 to 0.814 for the others); the multi-seed estimate is about half that but still excludes zero, and OPD's across-seed spread is half of SFT's. OPD on 100 questions (0.811) matches SFT on 900 (0.808), so the roughly 10x data-efficiency statement now rests on two budgets and three to four seeds.
