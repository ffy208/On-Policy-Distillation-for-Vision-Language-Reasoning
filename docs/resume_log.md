...
- Accomplished a seed-robust version of the low-data result, as measured by four seeds at 100 and 300 training questions (20 new training runs on L40S GPUs) giving OPD 0.811 (sd 0.009) versus SFT 0.793 (sd 0.022) at 100 questions and OPD 0.826 (sd 0.007) versus SFT 0.799 (sd 0.015) at 300, with paired bootstrap deltas of +1.8 [+0.4, +3.4] and +2.7 [+1.3, +4.2] points over the four seeds, by running the sweep through the idempotent experiment runner and aggregating with `python -m vlm_opd.analysis.data_efficiency` (intervals pooled over questions across seeds, deltas paired by seed and question).
- Recorded the correction honestly: the original single-seed +5.8 at 300 questions was the most favourable of four SFT seeds (0.778 against 0.800 to 0.814 for the others); the multi-seed estimate is about half that but still excludes zero, and OPD's across-seed spread is half of SFT's. OPD on 100 questions (0.811) matches SFT on 900 (0.808), so the roughly 10x data-efficiency statement now rests on two budgets and four seeds.
...
### Second task (2026-09-13, PACE)

- Accomplished the second-task test of the data-efficiency claim and found where it fails, as measured by a full Geometry3K curve (SFT and OPD at 100 / 300 / 900 / 2101 problems, 500 test problems, 1536-token budget) in which SFT beats OPD at three budgets (-5.2, -8.0, -7.0 points, paired intervals excluding zero) and ties at one, by running the same fixed-update protocol as on ChartQA on the cluster after fixing the truncation bug.
- Accomplished a mechanistic explanation from the evaluation records rather than a bare negative result, as measured by OPD students' format rate of 0.60 to 0.64 versus SFT's 0.72 to 0.80 with 139 to 153 of OPD's unformatted outputs hitting the token cap, OPD accuracy conditional on a formatted answer of 0.63 to 0.70 versus SFT's 0.57 to 0.63, a teacher that itself fails to terminate 22% of the time, and a rollout EOS rate flat at 0.5 to 0.6 across all 150 OPD steps, by adding per-record format and length breakdowns and reading the per-step training log. Conclusion recorded: OPD inherits the teacher's non-termination; SFT's correctness filter removes it.
- Accomplished confirmation of the mechanism with seeds and a control, as measured by four seeds at 300 and 900 problems (SFT 0.420 vs OPD 0.419 at 300, a tie; SFT 0.481 vs OPD 0.402 at 900, paired -7.9 [-10.0, -5.9]) and an unfiltered-SFT control whose format rate falls from 0.73 to 0.62 (identical to OPD's 0.62) at 300 and whose accuracy edge over OPD disappears there (+1.8, interval includes zero) while at 900 the filter accounts for 3.6 of the 7.9 points, by adding a keep-all option to teacher-solution generation, a control task, variant-aware runner naming, and a table that pairs every variant against SFT by seed.
...
- Raised student accuracy from 0.668 to 0.826 (4 seeds) on 500 held-out human-written questions using only 300 training questions, matching supervised distillation trained on 3,000 (0.834) and beating it by 2.7 points at equal data (paired bootstrap 95% CI +1.3 to +4.2, four seeds); OPD on 100 questions matches SFT on 900.
