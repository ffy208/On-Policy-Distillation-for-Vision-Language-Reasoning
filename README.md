...
| OPD on 300 questions (2400 rollouts) | 0.836 [0.804, 0.868] |
| Teacher, zero-shot | 0.844 [0.812, 0.876] |

1. **Is OPD stronger than SFT on multimodal reasoning?** In the low-data regime, yes: at 300 training questions OPD beats SFT by 5.8 points (paired bootstrap 95% CI +2.8 to +8.8). At 900 and 3000 questions both methods saturate at the teacher ceiling and the differences are within noise.
2. **Is OPD more data-efficient?** Yes. OPD with 10% of the questions matches SFT trained on all of them, reproducing the data-efficiency claim of Agarwal et al. (2024) in the vision-language setting.
...
Update counts are fixed across budgets so data quantity is the only variable: SFT runs 302 optimizer steps everywhere (2 epochs at 3000), OPD runs 150 steps of batch 16 everywhere. Caveats: a single seed; the 3000-question OPD run covers each question less than once, so it is undertrained relative to SFT's 2 epochs; the crossover lies at or below 300 questions because 300 was the smallest budget tested.
...
- [ ] Optional: 100-question point to locate the crossover; multi-seed repeat of the 300-question point; forward-KL ablation; longer OPD run at 3000 questions; Stage 5 self-distillation (SDPO)
