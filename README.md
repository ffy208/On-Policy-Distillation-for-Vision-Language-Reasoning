...
| Model | Format rate (has `Answer:`) | Accuracy given a formatted answer | Unformatted outputs that hit the 1536-token cap |
|---|---|---|---|
...
and OPD matches the teacher's distribution over the student's own long rollouts, so it inherits the habit: the
per-step log shows the EOS rate of the rollouts flat at 0.5 to 0.6 for all 150 steps and the KL on answer tokens
